...
int isExcludedWord(char *word) {
    const char *excludedWords[] = {
        "the", "of","at","and", "to", "in", "a", "is", "that", "it", "was", "with", "on", "for", "by", "as", "from"
...
int main() {
    ...
    while (fscanf(file, "%s", word) == 1) {
        ...
        if (!isDelimiter(word[0]) && !isExcludedWord(word)) {
    ...
    printf("The 10 most used words (excluding prepositions and nouns) in the text are:\n");
